...
ResourceCalculator::Quad::Quad(){
    isMatching[0] = new bool(false);
    isMatching[1] = new bool(false);
    isMatching[2] = new bool(false);
    isMatching[3] = new bool(false);
}

ResourceCalculator::Quad::~Quad() = default;

ResourceCalculator::Quad::Quad(const Quad &quad){
   isMatching = {new bool[4]};
   isMatching[0] = new bool(quad.isMatching[0]);
   isMatching[1] = new bool(quad.isMatching[1]);
   isMatching[2] = new bool(quad.isMatching[2]);
   isMatching[3] = new bool(quad.isMatching[3]);
...
int* ResourceCalculator::computeResources(ResourceTrails trail) {
    ...
    Quad *quad = new Quad();
           ...
           Quad cpy = Quad(*quad);
           map.insert(pair<vertex_t, Quad>(root, cpy));
    // delete r_resources
    delete quad;
    delete r_resources;
    ...
    while(!queue.empty()){
        ...
        for (; neighbourIt != neighbourEnd; ++neighbourIt) {
            ...
            Quad *next_quad = new Quad();
            next_quad->isMatching[0] = false;
            next_quad->isMatching[1] = false;
            next_quad->isMatching[2] = false;
            next_quad->isMatching[3] = false;
            ...
            if(direction == *DOWN){
                ...
                if(map.find(next_element) != map.end()) {
                    delete next_quad;
                    next_quad = new Quad(map[next_element]);
                }
                ...
                *next_quad->current_visit_count = *next_quad->current_visit_count +1;
                ...
                if(root_resources[0] == next_resources[1] && !next_quad->isMatching[1]){
                    next_quad->isMatching[1] = true;
                    ResourceTypes root_rs = root_resources[0];
                    ResourceTypes next_rs = next_resources[1];
                    addResources(resources, next_resources[1]);
                }
            ...
            if(map.find(next_element) == map.end()) {
                Quad cpy = Quad(*next_quad);
                map.insert(pair<vertex_t, Quad>(next_element, cpy));
                delete next_quad;
            }
